...
function AddPreDefinedIncident(props){

    const [predefinedincidentName,setPreDefinedIncidentName]=useState('')
    const [anyInjuries,setAnyInjuries]=useState('')
    const [injuryDescription,setInjuryDescription]=useState('')
    const [predefinedincidentError, setPreDefinedIncidentError] = useState('')
...
sx={{
    ...
    <h1>PreDefinedIncident Action</h1></div>
...
e.preventDefault()
    ...
    <div class = "injury-form" style = {{position:"relative",marginLeft:350,marginRight:0,top:0}}>
        <label>Enter PreDefinedIncident</label><br/>
        <input type = 'text' 
         onChange={
             (e)=>{
            ...
            if (nam.length < 4 || nam.length >= 20) {
              setPreDefinedIncidentError('name should be more than 4 charatcters')
            } 
            else 
            {
                setPreDefinedIncidentName(e.target.value)
                setPreDefinedIncidentError('')
            }
        }}/>

        <span style = {{ color: 'red'}} > {predefinedincidentError } </span><br/>
    </div>
    ...
    <Input type = 'submit' onClick={()=>{var injury={
        ...
        props.mydispatch(AddPreDefinedIncident(injury))
        console.log(injury)
    }}
...
export default connect(mapStateToProps, mapDispatchToProps)(AddPreDefinedIncident);